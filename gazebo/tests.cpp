...
// Lock base rotation DOF (Add dof in ArmPlugin.h file if off)
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts) // ConstContactsPtr returns all gazebo contacts in vector
{
	...
	{
		// see http://gazebosim.org/tutorials?tut=contact_sensor for contact sensor explanation
		// contacts = vector containing all gazebo contacts (nothing to do with contact sensor)
		...
		*/
		...
		if (strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) // collision1 = tube
		{
			if(DEBUG){printf("Gripper touched object!");}
...
bool ArmPlugin::updateAgent()
{
...
#if VELOCITY_CONTROL // #if is processed at compile time, if is processed at runtime
  ...
  // Limit joint range
	...
	/
	*/
  int jointIndex = action/2; // see above for explanation
    ...
    if(hasGroundContact)
		{
			...
			rewardHistory = REWARD_LOSS; // value of previous reward
			newReward     = true; // reward has been issued
			endEpisode    = true; // episode over
		}
		...
		*/
		...
		if(!hasGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				const float alpha = 0.8f; // might need adapting

				// compute the reward: smoothed moving average of distDelta ()
				avgGoalDelta  = avgGoalDelta * alpha + distDelta * (1.0f - alpha);
				if(DEBUG){printf("avgGoalDelta = %f\n", avgGoalDelta);}

				rewardHistory = avgGoalDelta; // might need adapting (reward falloff?)
				newReward     = true;
			}
